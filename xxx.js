      ...
      {application ? (
          ...
          {tabIndex === 0 && studentInfo && (
            <Box sx={{ padding: 2 }}>
              ...
              {[
                ...
                { label: "Date of Birth", value: new Date(studentInfo.dateOfBirth).toLocaleDateString() },
              ].map((item, index) => (
                <Box key={index} sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
                  <Typography variant="h6" sx={{ fontWeight: "bold" }}>{item.label}: {item.value}</Typography>
                  <Tooltip title="Copy">
                                ...
                                <Box sx={{ display: "flex", alignItems: "center" }}>
                                  {file.profileImageUrl ? (
                                    <CardMedia
                                      ...
                                      image={`https://localhost:7048/${file.profileImageUrl}`}
                                      alt={`${file.uploadedBy}`}
                                    />
                                  ) : (
        ...
        {file.createdAt ? new Date(file.createdAt).toLocaleString("en-GB", {
          ...
          <Modal open={openModal} onClose={() => setOpenModal(false)}>
            <Box sx={{ padding: 3, backgroundColor: "white", margin: "auto", marginTop: 10, maxWidth: 400 }}>
